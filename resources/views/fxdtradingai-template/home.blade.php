...
<html lang="en">
 <head>
  ...
  /* Style for inline flag in country notice */
  #notice-flag {
  ...
  /* Flag sprite positions for common countries */
  #notice-flag.iti__gb { background-position: -162px 0; } /* UK flag - correct position */
  #notice-flag.iti__us { background-position: -252px 0; }
  #notice-flag.iti__ph { background-position: -216px 0; }
  #notice-flag.iti__ae { background-position: 0 0; }
  #notice-flag.iti__il { background-position: -126px 0; }
  #notice-flag.iti__sg { background-position: -234px 0; }
  #notice-flag.iti__my { background-position: -180px 0; }
  #notice-flag.iti__in { background-position: -108px 0; }
  #notice-flag.iti__au { background-position: -36px 0; }
  #notice-flag.iti__ca { background-position: -54px 0; }
  #notice-flag.iti__de { background-position: -72px 0; }
  #notice-flag.iti__fr { background-position: -90px 0; }
  #notice-flag.iti__es { background-position: -81px 0; }
  #notice-flag.iti__it { background-position: -117px 0; }
  #notice-flag.iti__nl { background-position: -198px 0; }
  #notice-flag.iti__be { background-position: -45px 0; }
  #notice-flag.iti__at { background-position: -27px 0; }
  #notice-flag.iti__ch { background-position: -63px 0; }
  #notice-flag.iti__se { background-position: -243px 0; }
  #notice-flag.iti__no { background-position: -207px 0; }
  #notice-flag.iti__dk { background-position: -78px 0; }
  #notice-flag.iti__fi { background-position: -99px 0; }
  #notice-flag.iti__pl { background-position: -225px 0; }
  #notice-flag.iti__ie { background-position: -144px 0; }
  #notice-flag.iti__pt { background-position: -231px 0; }
  #notice-flag.iti__ro { background-position: -237px 0; }
  #notice-flag.iti__hu { background-position: -135px 0; }
  #notice-flag.iti__gr { background-position: -108px 0; }
  #notice-flag.iti__bg { background-position: -54px 0; }
  #notice-flag.iti__mx { background-position: -189px 0; }
  #notice-flag.iti__br { background-position: -54px 0; }
  #notice-flag.iti__ar { background-position: -27px 0; }
  #notice-flag.iti__cl { background-position: -72px 0; }
  #notice-flag.iti__co { background-position: -81px 0; }
  #notice-flag.iti__za { background-position: -270px 0; }
  #notice-flag.iti__ng { background-position: -198px 0; }
  #notice-flag.iti__eg { background-position: -90px 0; }
  #notice-flag.iti__jp { background-position: -153px 0; }
  #notice-flag.iti__kr { background-position: -171px 0; }
  #notice-flag.iti__th { background-position: -255px 0; }
  #notice-flag.iti__vn { background-position: -267px 0; }
  #notice-flag.iti__tr { background-position: -261px 0; }
  #notice-flag.iti__sa { background-position: -243px 0; }
 </style>
   ...
   <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
    <article class="prose lg:prose-lg lg:col-span-2">
     ...
     <div class="bg-white px-6 sm:px-12 py-6 sm:py-8 rounded-lg shadow-lg max-w-md w-full mx-auto mb-6 sm:mb-8" id="contact">
      ...
      <form action="{{ route('leads.store') }}" class="form-registration space-y-4 sm:space-y-6" method="POST" onsubmit="return validateForm();">
       @csrf
       ...
       <div class="space-y-3 sm:space-y-4 hidden" id="phone-section">
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:gap-3">
         <input class="phone flex-1 w-full px-4 py-2 sm:py-3 rounded-lg border border-gray-300 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm" id="phoneNumber" name="phone_number" placeholder="Phone Number" type="tel" data-force-country="{{ $forceCountry ? '1' : '0' }}" title="{{ $forceCountry ? ($computedIso === 'GB' ? 'e.g., 07123 456789' : ($computedIso === 'US' ? 'e.g., 555-123-4567' : ($computedIso === 'IL' ? 'e.g., 050-123-4567' : ($computedIso === 'AE' ? 'e.g., 050-123-4567' : 'e.g., 917-123-4567')))) : '' }}"/>
        </div>
        <!-- Country notice -->
        <div class="mt-1 text-xs text-gray-500 flex items-center gap-1" id="country-notice">
         <span>
          Currently only 
      ...
      <!-- ... -->
      <!-- intl-tel-input script (load before our init script) -->
      <script src="https://cdn.jsdelivr.net/npm/intl-tel-input@17.0.19/build/js/intlTelInput.min.js?v=17.0.19"></script>
      <script src="https://cdn.jsdelivr.net/npm/intl-tel-input@17.0.19/build/js/utils.js?v=17.0.19"></script>
      <script>
       // Phone field behavior: use only intl-tel-input, remove legacy dropdown, sync hidden fields
       document.addEventListener('DOMContentLoaded', function() {
         var phoneInput = document.getElementById('phoneNumber');
         var hiddenDial = document.getElementById('area_code_hidden');
         var hiddenIso = document.querySelector('input[name="country"]');
         ...
         var iso = (metaIsoEl && metaIsoEl.getAttribute('content') || '').toUpperCase();
         var isForced = metaForceEl && metaForceEl.getAttribute('content') === '1';

         // Remove the old country dropdown if present (UI cleanup)
         try { var select = document.getElementById('countryCode'); if (select && select.parentNode) select.parentNode.removeChild(select); } catch(e){}

         var itiInstance = null;
         try {
           if (window.intlTelInput && phoneInput) {
             itiInstance = window.intlTelInput(phoneInput, {
               initialCountry: iso ? iso.toLowerCase() : 'ph',
               separateDialCode: true,
               nationalMode: true,
               autoPlaceholder: 'polite'
             });
           }
         } catch (e) { /* ignore plugin init failure */ }
         ...
         // Initialize sync from plugin
        updateNotice();
        
        // Explicitly set the initial flag based on computed ISO
        try {
          var initialIso = '{{ $computedIso }}';
          if (noticeFlag && initialIso) {
            // Clear any existing country classes
            Array.from(noticeFlag.classList).forEach(function(c){ if (c.indexOf('iti__')===0 && c!=='iti__flag') noticeFlag.classList.remove(c); });
            // Add the correct country class
            noticeFlag.classList.add('iti__' + initialIso.toLowerCase());
          }
        } catch(e){}
         if (phoneInput && itiInstance) {
           phoneInput.addEventListener('countrychange', updateNotice);
         }

         // If country is forced, lock the picker and align to ISO
         if (isForced) {
           try { if (iso && itiInstance && itiInstance.setCountry) itiInstance.setCountry(iso.toLowerCase()); } catch(e){}
           try {
             var selectedFlag = document.querySelector('.iti__selected-flag');
             if (selectedFlag) selectedFlag.addEventListener('click', function(ev){ ev.preventDefault(); ev.stopPropagation(); });
           } catch(e){}
           updateNotice();
         }
       });
      ...
      <!-- ... -->
      <script>
       // Email/Phone validation and AJAX submit (design-agnostic)
       (function(){
         ...
         var el = {
           ...
           alert: document.getElementById('feedback-message'),
           backBtn: document.getElementById('back-button'),
           initialForm: document.getElementById('initial-form'),
           phoneSection: document.getElementById('phone-section')
         };
         function showAlert(msg){ if(!el.alert) return; el.alert.textContent = msg; el.alert.classList.remove('hidden'); el.alert.classList.add('text-red-600'); }
         function showSuccess(msg){ if(!el.alert) return; el.alert.textContent = msg; el.alert.classList.remove('hidden'); el.alert.classList.remove('text-red-600'); el.alert.classList.add('text-green-600'); }
         function clearAlert(){ if(!el.alert) return; el.alert.textContent=''; el.alert.classList.add('hidden'); }
         // Back button handler
         if (el.backBtn) {
           el.backBtn.addEventListener('click', function(){
             if (el.initialForm && el.phoneSection) {
               el.phoneSection.classList.add('hidden');
               el.initialForm.classList.remove('hidden');
               try { el.email && el.email.focus(); } catch(e){}
               // hide back button and clear any feedback
         ...
         function validateEmail(){
           if(!el.email) return true; var v=(el.email.value||'').trim(); return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(v);
         }
         function getActiveIso(){ var m=document.querySelector('meta[name="isoCode"]'); return (m && m.getAttribute('content')||'').toUpperCase(); }
         function validatePhone(){
           if(!el.phone) return true; var v=String(el.phone.value||''); var digits=v.replace(/\D/g,''); var iso=getActiveIso(); var ok;
           switch(iso){
             case 'GB': ok=/^(0?7\d{9})$/.test(digits); break; // UK mobile
             case 'US': ok=/^\d{10}$/.test(digits); break;     // NANP 10 digits
             case 'IL': ok=/^(0?5\d{8})$/.test(digits); break; // IL mobile
             case 'AE': ok=/^(0?5\d{7,8})$/.test(digits); break; // AE mobile
             default: ok = digits.length>=6 && digits.length<=14;
           }
           return ok;
         }
         function submitLeadAjax(){
               ...
               if (!r.ok){
                 ...
                 try{
                   ...
                   if (r.status===422 && payload && payload.errors){
                     if (payload.errors.email) showAlert(payload.errors.email[0]||'Invalid email address.');
                     else if (payload.errors.phone_number || payload.errors.phone) {
                       var msg=(payload.errors.phone_number && payload.errors.phone_number[0]) || (payload.errors.phone && payload.errors.phone[0]) || 'Invalid phone number.'; showAlert(msg);
                     } else if (payload.errors['cf-turnstile-response']){
                       showAlert(payload.errors['cf-turnstile-response'][0] || 'Please verify that you are human.');
                       try { if (window.turnstile && typeof window.turnstile.reset==='function') window.turnstile.reset(); } catch(e){}
                     } else { var firstKey=Object.keys(payload.errors)[0]; var firstMsg=firstKey ? (payload.errors[firstKey][0]||'') : ''; showAlert(firstMsg||'Please check your input and try again.'); }
                     // Show back button so user can edit email
                     if (el.backBtn) el.backBtn.classList.remove('hidden');
                     btn && (btn.disabled=false); return Promise.reject();
                   }
                   if (r.status===429){ showAlert('Too many attempts. Please wait a moment and try again.'); btn && (btn.disabled=false); return Promise.reject(); }
                   showAlert('Submission failed. Please try again.'); btn && (btn.disabled=false); return Promise.reject();
                 } catch(e){ showAlert('Submission failed. Please try again.'); btn && (btn.disabled=false); return Promise.reject(); }
               }
               return r.json();
             })
             .then(function(data){
               showSuccess('Thank you for sharing your information with us! We\'ll reach out within 48 hours.');
         ...
         window.validateForm = function(){
           clearAlert();
           var okE = validateEmail();
           var okP = validatePhone();
           if (!(okE && okP)){
             showAlert(!okE ? 'Invalid email address.' : 'Invalid phone number.');
             // Show back button so user can edit email when on phone section
             if (el.backBtn) el.backBtn.classList.remove('hidden');
             return false;
           }
           submitLeadAjax();
           return false; // always prevent default submit
         };
      })();
     </script>
